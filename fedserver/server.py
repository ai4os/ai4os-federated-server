...
FEDERATED_MIN_AVAILABLE_CLIENTS: int = int(
    os.environ["FEDERATED_MIN_AVAILABLE_CLIENTS"]
)
...
class AggregateEmissions(type(strategy)):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        client_emissions = [
            res.metrics["emissions"] for _, res in results if "emissions" in res.metrics
        ]
        ...
        print(
            f"ROUND {server_round}. Total Carbon Emissions (server and clients): {total_emissions} kg CO2"
        )
